...
SELECT tbl.slotid, tbl.classid, tbl.teacherid, tbl.teachername ,tbl.subjectid,tbl.subjectname,tbl.dayid,tbl.daylabel,tbl1.label  FROM   time_slot  tbl1 LEFT JOIN  time_table  tbl ON tbl.slotid = tbl1.slotid    AND tbl.classid =2   ORDER BY label ASC 
       "readOnlyTimeTable" :[["8-9","Hindi","History","Computer","RECESS1","RECESS2"],]
		...
		JSONArray jSlotArray = new JSONArray();
		try {
			...
			//FETCH CLASS
			sql = "SELECT tbl.slotid, tbl.classid, tbl.teacherid, tbl.teachername ,tbl.subjectid,tbl.subjectname,tbl.dayid,tbl.daylabel,tbl1.label  FROM   time_slot  tbl1 LEFT JOIN  time_table  tbl ON tbl.slotid = tbl1.slotid    AND tbl.classid =2   ORDER BY label ASC";
			...
			*/
			...
			while(rs.next())
			{
				...
				if(rs.getString(6) != null && rs.getString(6).equals("BREAK"))
				{
					
					//break
					JSONArray jArr =  new JSONArray(); 
					jArr.put(rs.getString(9));
					jArr.put("");
					jArr.put("");
					jArr.put("BREAK");
					jArr.put("");
				...
				else
				{
					...
					{
						//is OBJECT EXIST
						int k = (Integer) slotWise.get(key+"");
						JSONArray jResp = (JSONArray) jSlotArray.get(k);
						jResp.put(rs.getString(6));
						jSlotArray.put(k,jResp);
					//	slotWise.put(key+"",jResp);
					}
					else
					{
						JSONArray jArr =  new JSONArray(); 
						jArr.put(rs.getString(9));
						jArr.put(rs.getString(6));
						int kIndex = jSlotArray.length()-1;
								if(kIndex <= 0)
									kIndex = 0;
						slotWise.put(key+"",kIndex);
						jSlotArray.put(jArr);
					}
				}
			}
		...
		try {
			returnableJSON.put("readOnlyTimeTable", jSlotArray);
		} catch (JSONException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
